...
# CRITICAL: must include schema + engine (.prisma) + CLI (prisma package) + runtime (@prisma)
...
# Start command: sync DB schema → seed → start server
CMD sh -c "\
  mkdir -p /app/data /app/public/uploads && \
  export DATABASE_URL=file:/app/data/qrtrans.db && \
  echo '>>> [1/3] Syncing DB schema with prisma db push...' && \
  npx prisma db push --skip-generate --accept-data-loss 2>&1 && \
    echo '>>> [1/3] DB schema synced OK' || \
    echo '>>> [1/3] WARNING: prisma db push failed' ; \
  echo '>>> [2/3] Running seed...' ; \
  bun run prisma/seed.ts 2>&1 || echo '>>> [2/3] Seed skipped (non-critical)' ; \

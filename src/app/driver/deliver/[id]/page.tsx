...
export default function DriverDeliverPage() {
  ...
  // ─── WhatsApp links from delivery confirmation API (unmasked) ──
  const [waSender, setWaSender] = useState<string | null>(null);
  const [waReceiver, setWaReceiver] = useState<string | null>(null);
  const [deliverySenderName, setDeliverySenderName] = useState<string>('');
  const [deliveryReceiverName, setDeliveryReceiverName] = useState<string>('');
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        // Capture unmasked WhatsApp links from API response
        setWaSender(data.wa_sender || null);
        setWaReceiver(data.wa_receiver || null);
        setDeliverySenderName(data.sender?.name || parcel?.travelerFirstName || '');
        setDeliveryReceiverName(data.receiver?.name || parcel?.receiverName || '');
          ...
          <div className="w-full space-y-3 mb-8">
            ...
            {waSender && (
              <a
                ...
                className="flex items-center gap-3 w-full bg-[#25D366]/15 border-2 border-[#25D366]/40 rounded-2xl px-5 py-4 hover:bg-[#25D366]/25 active:scale-[0.98] transition-all"
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-white">Notifier l&apos;expéditeur</p>
                  <p className="text-[11px] text-[#25D366]/70">
                    {deliverySenderName || 'Expéditeur'} — WhatsApp
                  </p>
                </div>
                <svg className="w-5 h-5 text-[#25D366]/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </a>
            )}
            ...
            {waReceiver && (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-white">Notifier le destinataire</p>
                  <p className="text-[11px] text-[#25D366]/70">
                    {deliveryReceiverName || 'Destinataire'} — WhatsApp
                  </p>
                </div>
                <svg className="w-5 h-5 text-[#25D366]/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </a>
